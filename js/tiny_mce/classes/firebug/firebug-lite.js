      ...
      openHtmlTree:function(_element,_parent,_returnParentElementByElement,_event){
        with(firebug){
          ...
          if(element==document.documentElement){
            new lib.element("A").attribute.addClass("Blogcategory").update("<span class='DarkBlue'>&#60;<span class='Blue'>html</span>&#62;").insert(parent);
          };

          for(var i=0; i<=len; i++){
            if(i==len){
              new lib.element("A").attribute.addClass("Blogcategory").update("<span class='DarkBlue'>&#60;/<span class='Blue'>"+element.nodeName.toLowerCase()+"</span>&#62;").insert(container);
              break;
            } 
            var item = element.childNodes[i];

            if (item.nodeType != 3){
              var container = new lib.element().attribute.addClass("Blogcategory").insert(parent),
    ...
    xhr:{
      ...
      open:function(){
        with(firebug){
          ...
          el.left.xhr.name = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").attribute.addClass("Blogcategory").environment.addStyle({ "width":"20%" }).insert(el.left.xhr.container));
          ...
          el.left.xhr.status = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").attribute.addClass("Blogcategory").environment.addStyle({ "width":"10%" }).insert(el.left.xhr.container));
          ...
          el.left.xhr.readystate = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").environment.addStyle({ "width":"15%" }).attribute.addClass("Blogcategory").insert(el.left.xhr.container));
          ...
          el.left.xhr.response = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").environment.addStyle({ "width":(lib.env.ie?"50":"55")+"%" }).attribute.addClass("Blogcategory").insert(el.left.xhr.container));
